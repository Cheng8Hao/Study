...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    //liyang
    private static  boolean isFactoryTestInto =false;
    ...
    public void onCreate() {
        ...
        openDevice();
        android.util.Log.d("dyc_fm","openDevice()==="+openDevice());
    ...
    private synchronized void createAudioPatchByEarphone() {
        Log.d(TAG, "createAudioPatchByEarphone " + mIsPlaying192KHz);
        if (mIsPlaying192KHz) {
            releaseAudioPatch();
            if (!mIsRender) {
                startRender();
            ...
            if (mAudioPatch != null) {
                ...
                if (isPatchMixerToEarphone(patches)) {
                    ...
                    if (isOutputDeviceChanged(patches)) {
                        Log.d(TAG, "DEBUG outputDeviceChanged: re-create audio patch");
                        releaseAudioPatch();
                        if ("1".equals(SystemProperties.get("ro.mtk_vibspk_support", "0"))) {
                            mIsSpeakerUsed = true;
                            notifySpeakerModeChange();
                ...
                if (isPatchMixerToEarphone(patches)) {
                    ...
                    if ("0".equals(SystemProperties.get("ro.mtk_vibspk_support", "0"))) {
                        stopRender();
                        stopAudioTrack();
                        createAudioPatchBySpeaker();
                        mIsSpeakerUsed = true;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent != null && intent.hasExtra("FactoryStartFM")) {       
           isFactoryTestInto = true;
           boolean hasFactoryPowerup=false;
           while(true){
                   android.util.Log.d("rongweidi123","start");
                   if(mAudioManager.isMusicActive()){
                           if(!isDeviceOpen()){
                                   android.util.Log.d("rongweidi123","openDevice");
                                   openDevice();
                           }
                           android.util.Log.d("rongweidi123","powerUpAsync");
                           powerUpAsync((float) 96.2);
                   }else if(!hasFactoryPowerup){
                           hasFactoryPowerup=true;
                           powerUpAsync((float) 96.2);     
                           try {
                                   Thread.sleep(200);
                           } catch (InterruptedException e) {
                   ...
                   android.util.Log.d("rongweidi123","end");
                   try {
                           Thread.sleep(200);
                   } catch (InterruptedException e) {
                           // TODO Auto-generated catch block
                   ...
                   if(mPowerStatus == POWER_UP){
                           android.util.Log.d("rongweidi123","powerUpAsync success");
                           break;
                   }
                   android.util.Log.d("rongweidi123","powerUpAsync fail");
           }
           fmDefaultVolume = mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC); 
           android.util.Log.d("jiangcunbin123","fmDefaultVolume1 = "+fmDefaultVolume);
           setSpeakerPhoneOn(false);
           setSpeakerPhoneOn(true);
           int volume = mAudioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
           mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, volume, 0);
            }else if(intent != null && intent.hasExtra("FactoryCloseFM")){
               isFactoryTestInto = false;
               setSpeakerPhoneOn(false);
    ...
    private boolean firstPlaying(float frequency) {
        ...
        float seekStation = frequency;
        if(!isFactoryTestInto){
