...
public class FmEmActivity extends Activity {
    ...
    // Can not use float to record the station. Because there
    // will be inaccuracy when increase/decrease 0.1
        ...
        public void onClick(View v) {
            ...
            case R.id.FMR_Freq_tune:
                ...
                try {
                    ...
                    int station = FmUtils.computeStation(freq);
                    if (!FmUtils.isValidStation(station)) {
                        Toast.makeText(FmEmActivity.this, "Please input 87.5 ~ 108.0.",
                                Toast.LENGTH_SHORT).show();
                        mEditFreq.setText(FmUtils.formatStation(mCurrentStation));
                        return;
                    }
                } catch (NumberFormatException e) {
                    Toast.makeText(FmEmActivity.this, "bad float format.",
                            Toast.LENGTH_SHORT).show();
                    mEditFreq.setText(FmUtils.formatStation(mCurrentStation));
                    return;
                }
                ...
                tuneToStation(FmUtils.computeStation(freq));
                break;
            case R.id.FMR_EM_RSSI_THRESHOLD_OK:
                ...
                try {
                    ...
                    Toast.makeText(FmEmActivity.this, "bad format.", Toast.LENGTH_SHORT).show();
                    mEditFreq.setText("0");
                    return;
                }
                ...
                if (isRssiOk) {
                    Toast.makeText(FmEmActivity.this, "Rssi Threshold set OK.",
                            Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(FmEmActivity.this, "Rssi Threshold set fail.",
                            Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.FMR_EM_DESENSE_RSSI_OK:
                ...
                if (isDeRssiOk) {
                    Toast.makeText(FmEmActivity.this, "Desense Rssi Threshold set OK.",
                            Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(FmEmActivity.this, "Desense Rssi Threshold set fail.",
                            Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.FMR_EM_MUTE_GAIN_OK:
                ...
                try {
                    ...
                    Toast.makeText(FmEmActivity.this, "bad format.",
                            Toast.LENGTH_SHORT).show();
                ...
                if (isMuteOk) {
                    Toast.makeText(FmEmActivity.this, "Software mute gain set OK.",
                            Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(FmEmActivity.this, "Software mute gain set fail.",
                            Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.FMR_EM_CMD_OK:
                ...
                if (cmds == null) {
                    Toast.makeText(FmEmActivity.this, "cmd execute fail.",
                            Toast.LENGTH_SHORT).show();
     ...
     * FM Radio listener
     */
    private FmListener mFmListener = new FmListener() {

        /**
         * call back method from service
         */
        public void onCallBack(Bundle bundle) {
            Message msg = mHandler.obtainMessage(bundle.getInt(FmListener.CALLBACK_FLAG));
            msg.setData(bundle);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //FmStation.initFmDatabase(mContext);
    ...
    private ServiceConnection mServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            Log.d(TAG, "begin FmEmActivity.onServiceConnected");
            ...
            //mService.regisetFmRadioListener(mFmListener);
            ...
            Log.d(TAG, "<<< FmEmActivity.onServiceConnected");
        }

        public void onServiceDisconnected(ComponentName className) {
            Log.d(TAG, ">>> FmEmActivity.onServiceDisconnected");
            mService = null;
            Log.d(TAG, "<<< FmEmActivity.onServiceDisconnected");
        }
    };
...
//                FmStation.setCurrentStation(mContext, mCurrentStation);
            ...
            case FmListener.LISTEN_RT_CHANGED:
                bundle = msg.getData();
                mPSString = bundle.getString(FmListener.KEY_PS_INFO);
                mLRTextString = bundle.getString(FmListener.KEY_RT_INFO);
                showRDS();
                break;
    ...
    protected void onStart() {
        super.onStart();
        mIsServiceBinded = bindService(new Intent(FmEmActivity.this, FmService.class),
...
//            powerDown();
    ...
    private void showRDS() {
        Log.v(TAG, "FmEmActivity.showRDS PS : " + mPSString + "RT :" + mLRTextString);
    ...
    private boolean isPowerUp() {
        boolean bRet = false;
        if (null != mService) {
            return mService.getPowerStatus() == FmService.POWER_UP;
