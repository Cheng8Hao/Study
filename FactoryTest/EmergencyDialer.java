...
//duanyecong add for factorymode 2018.7.30 begin
import android.content.pm.PackageManager;
import android.content.ComponentName;
//duanyecong add for factorymode 2018.7.30 end
...
public class EmergencyDialer extends Activity implements View.OnClickListener,
    ...
    ///M: Subsidy lock feature for OP18 @{
    private StatusBarManager mStatusBarManager;
    private static final String MTK_SECRET_CODE_INTENT =
                                   "com.mediatek.intent.action.emergency.SECRET_CODE";
    private static final String ACTION_SUBSIDY_LOCK_STATE_CHANGE =
                                   "com.mediatek.subsidy_lock.state_change";
    private static final int APP_LOCK_STATE = 0;
    private int mStatusBarLockState = -1;
    private boolean mIsSupportSubsidyLock;
    ///@}
    EditText mDigits;
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Intent.ACTION_SCREEN_OFF.equals(intent.getAction())) {
                finishAndRemoveTask();
              ///M: Subsidy lock feature for OP18 @{
            } else if (ACTION_SUBSIDY_LOCK_STATE_CHANGE.equals(intent.getAction())) {
                int state = intent.getIntExtra("state", -1);
                Log.d(LOG_TAG, "[SL] Receiving SUBSIDY_LOCK_STATE_CHANGE with state :" + state);
                if (state == APP_LOCK_STATE) {
                    Log.d(LOG_TAG, "[SL] Update ECC Dialer for Subsidy lock");
                    updateEccDialerForSubsidy();
                }
            }
              ///@}
        }
    ...
    public void afterTextChanged(Editable input) {
        ///M: Subsidy lock feature for OP18 @{
        if (mIsSupportSubsidyLock && "*#5625".equals(input.toString())) {
            Intent intent = new Intent(MTK_SECRET_CODE_INTENT);
            Log.d(LOG_TAG, "[SL] send SECRET CODE ACTION broadcast with *#5625");
        ...
        if("*96#".equals(input.toString())){
           // duanyecong factorymode to skip setup wizard 2018.7.30 begin
            int provisioned = android.provider.Settings.Global.getInt(this.getContentResolver(),android.provider.Settings.Global.DEVICE_PROVISIONED, 0);
            int complete = android.provider.Settings.Secure.getInt(this.getContentResolver(),android.provider.Settings.Secure.USER_SETUP_COMPLETE, 0);
            if (provisioned + complete == 0){
                PackageManager pm = this.getPackageManager();
            try{
                pm.setApplicationEnabledSetting("com.google.android.setupwizard",PackageManager.COMPONENT_ENABLED_STATE_DISABLED,PackageManager.DONT_KILL_APP);
            }catch(Exception e) {
            
            }finally {
                android.provider.Settings.Global.putInt(this.getContentResolver(), android.provider.Settings.Global.DEVICE_PROVISIONED, 1);
                android.provider.Settings.Secure.putInt(this.getContentResolver(), android.provider.Settings.Secure.USER_SETUP_COMPLETE, 1);
                mDigits.getText().delete(0, mDigits.getText().length());
                //sendOrderedBroadcast(new Intent("android.dyc.action.FACTORY.START"),null);
                //startActivity(new Intent("android.intent.action.MAIN", null).addCategory("android.intent.category.HOME").addFlags(270565376));
                try {
          ...
          //duanyecong factorymode to skip setup wizard 2018.7.30 end
        /// @}
    ...
    protected void onCreate(Bundle icicle) {
        ...
        ExtensionManager.getEmergencyDialerExt().onCreate(this);
        ///  @}
        ...
        ///M: Subsidy lock feature for OP18
        intentFilter.addAction(ACTION_SUBSIDY_LOCK_STATE_CHANGE);
    ...
    protected void onDestroy() {
        ...
        ///M: add for CT plug in. @{
        ExtensionManager.getEmergencyDialerExt().onDestroy(this);
        /// @}
    }
    ...
    protected void onStop() {
        ...
        if (mStatusBarLockState != -1) {
            mStatusBarManager.disable(StatusBarManager.DISABLE_NONE);
            Log.d(LOG_TAG, "[SL] onStop mStatusBarLockState = " + mStatusBarLockState);
            mStatusBarLockState = -1;
        }
    ...
    ///M: Subsidy lock feature for OP18 @{
    private void updateEccDialerForSubsidy() {
       boolean subsidyLockProp = SystemProperties.get("ro.mtk_subsidy_lock_support", "0")
                                        .equals("1");
       boolean subsidyLockStatus = SystemProperties.get("persist.subsidylock").equals("0");
       mIsSupportSubsidyLock = subsidyLockProp && subsidyLockStatus;
       Log.d(LOG_TAG, "[SL] Subsidy Lock Property :" + subsidyLockProp
                        + " subsidy Lock Status :" + subsidyLockStatus);
       if (mIsSupportSubsidyLock && mStatusBarLockState == -1) {
            mStatusBarLockState = StatusBarManager.DISABLE_RECENT
                                | StatusBarManager.DISABLE_HOME
                                | StatusBarManager.DISABLE_EXPAND
                                | StatusBarManager.DISABLE_NOTIFICATION_ICONS
                                | StatusBarManager.DISABLE_NOTIFICATION_ALERTS
                                | StatusBarManager.DISABLE_NOTIFICATION_TICKER;
            mEmergencyActionGroup.setVisibility(View.INVISIBLE);
            Log.d(LOG_TAG, "[SL] onResume mSubsidyStatusBarLockState = " + mStatusBarLockState);
            mStatusBarManager.disable(mStatusBarLockState);
        }
    }
    ///@}
}
