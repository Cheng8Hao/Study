...
/// M: MTK STK feature
import com.mediatek.server.stk.StkManager;
...
//duanyecong add for factorymode begin
import android.app.ActivityManager.RunningTaskInfo;
//duanyecong add for factorymode end
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /// M: MTK STK feature
    private StkManager mStkManager = MtkSystemServiceFactory.getInstance().makeStkManager();
    ...
    //duanyecong add for factorymode begin
    private boolean isFactoryTestTop() {
        ComponentName topActivity = getTopActivity();
        ComponentName baseActivity = getBaseActivity();
        String packageName = "";
        String className = "";
        ...
        if(topActivity != null){
            ...
            basePackageName = baseActivity.getPackageName().trim();
        }
        if ("com.dyc.factorymode".equalsIgnoreCase(packageName)) {
            if ("com.dyc.factorymode.KeyTestActivity".equalsIgnoreCase(className)) {
                return true;
            }
            if ("com.dyc.factorymode.TouchTestActivity".equalsIgnoreCase(className)) {
                return true;
            }
            if ("com.dyc.factorymode.DiagonalTestActivity".equalsIgnoreCase(className)) {
                return true;
            }
        }
        return false;
    ...
    private ComponentName getBaseActivity() {
        final ActivityManager am = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
        ActivityInfo aInfo = null;
        List<RunningTaskInfo> list = am.getRunningTasks(1);
        if (list.size() != 0) {
            RunningTaskInfo baseRunningTask = list.get(0);
            return baseRunningTask.baseActivity;
        } else {
            return null;
        }
    }
    ...
    public void beginLayoutLw(boolean isDefaultDisplay, int displayWidth, int displayHeight,
        ...
        if (isDefaultDisplay) {
            ...
            if (mWmsExt.isFullscreenSwitchSupport()) {
                if (mWmsExt.isFocusWindowReady(mFocusedWindow)) {
    ...
    public void layoutWindowLw(WindowState win, WindowState attached) {
        ...
        Rect crop = null;
        if (mWmsExt.isFullscreenSwitchSupport()) {
            //compute and apply the black area region.
            crop = mWmsExt.getSwitchFrame(win, mFocusedWindow,
                    mOverscanScreenWidth, mOverscanScreenHeight);
            if (crop != null && (crop.top != 0 || crop.left != 0)) {
                updateRect(crop.left, crop.top, crop.right, crop.bottom);
            }
        }
        /// @}
        ...
        if (!isDefaultDisplay) {
           ...
           layoutWallpaper(win, pf, df, of, cf);
            ...
            if ((fl & (FLAG_LAYOUT_IN_SCREEN | FLAG_LAYOUT_INSET_DECOR))
                ...
                /// M: Add more log at WMS
                if (DEBUG_LAYOUT) {
                    mWindowManagerDebugger.debugLayoutWindowLw(TAG, adjust, attrs.type, fl,
                            canHideNavigationBar(), sysUiFl);
                }
    ...
    public void applyPostLayoutPolicyLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        /// M: Add more log at WMS
        if (DEBUG_LAYOUT) {
            mWindowManagerDebugger.debugApplyPostLayoutPolicyLw(TAG, win, attrs,
                    mTopFullscreenOpaqueWindowState, attached, imeTarget,
                    mDreamingLockscreen, mShowingDream);
        }
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if(isFactoryTestTop()) {
                    ...
                    powerIntent.setAction("com.sagereal.keycode.power");
    ...
    public void userActivity() {
        ...
        /// M: MTK STK feature @{
        mStkManager.sendStkBroadcast(mContext, mHandler);
        /// @}
